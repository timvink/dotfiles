#!/bin/bash
# run_onchange_setup_ghostty_terminfo.sh — install the xterm-ghostty terminfo
# entry on this (Linux) box.
#
# Why: snacks.image only renders inline when the terminal is detected as Ghostty,
# and dot_bashrc downgrades TERM to xterm-256color whenever the xterm-ghostty
# terminfo entry is missing — which silently disables image preview. Ghostty's
# own `ssh-terminfo` shell integration installs this entry on connect, but ONLY
# for the shell's `ssh`; the devbox is reached via `az ssh vm`, a subprocess that
# bypasses the Ghostty ssh wrapper, so the entry never arrives that way. Install
# it here from a vendored `infocmp -x xterm-ghostty` (regenerate that on a Mac
# with Ghostty if it ever changes) — connection-method-agnostic and idempotent.
# This run_onchange script re-runs automatically when the embedded entry changes.
#
# Managed by chezmoi (Linux only).
set -euo pipefail

# Already available (e.g. Ghostty's ssh-terminfo beat us to it) -> nothing to do.
if infocmp xterm-ghostty >/dev/null 2>&1; then
    exit 0
fi

if ! command -v tic >/dev/null 2>&1; then
    echo "setup_ghostty_terminfo: tic not found (install ncurses-bin); skipping." >&2
    exit 0
fi

echo "=== chezmoi: installing xterm-ghostty terminfo ==="
tmp=$(mktemp)
cat > "$tmp" <<'GHOSTTY_TERMINFO'
...
rm -f "$tmp"
echo "=== chezmoi: xterm-ghostty terminfo ready ==="
